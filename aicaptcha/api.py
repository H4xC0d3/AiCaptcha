...
class AiCaptcha:
	...
	def Hcaptcha(self, site_key: str, page_url: str, user_agent: str, version: str=None, widget_id: str=None, proxy: dict=None) -> hcaptcha:
		return hcaptcha(self.__request({"task": {"site_key": site_key, "page_url": page_url, "user_agent": user_agent, "version": version, "widget_id": widget_id, "proxy": proxy}, "type": "Hcaptcha"}))
	
	def ReCaptchaV3(self, site_key: str, page_url: str, action: str, type: str="normal", user_agent: str=None, cb: str=None, cookies=None, v: str=None, co: str=None, domain: str=None, proxy: dict=None) -> recaptchav3:
		return recaptchav3(self.__request({"task": {"type": type, "site_key": site_key, "page_url": page_url, "action": action, "user_agent": user_agent, "cb": cb, "cookies": cookies, "v": v, "co": co, "domain": domain, "proxy": proxy}, "type": "ReCaptchaV3"}))
