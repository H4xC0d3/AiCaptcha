...
class AiCaptcha:
	...
	def ObjectRecognition(self, image: str) -> objectrecognition_class:
		return objectrecognition_class(self.__request({"task": {"image_base64": self.__get_file(image)}, "type": "ObjectRecognition"}))
...
#		return ocr(self.__request({"task": {"image": self.__get_file(image)}, "type": "OCR"}))
	...
	def __get_file(self, content):
		...
		elif isinstance(content, str) and content.startswith("http") and "://" in content:
